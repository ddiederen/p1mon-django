...
from history.models import HistoryDag,HistoryMaand
...
#%% data query
# length of record
deltaT = timedelta(days=93)
datetime_query = timezone.now().date() - deltaT

# query set
qs1 = HistoryDag.objects.filter(timestamp__gte=datetime_query)
...
# drop columns
df1HistMonth2.columns
df1HistMonth2.drop(columns=['act_verbr_gas', 'act_verbr_kw_170', 'act_gelvr_kw_270',],inplace=True)

# drop first line with NAN values and incomplete last line
df1HistMonth2 = df1HistMonth2[1:-1]
...
print("Converted to history: months")