...
from history.models import HistoryMaand,HistoryJaar
...
#%% data query
# length of record
deltaT = timedelta(days=1000)
datetime_query = timezone.now().date() - deltaT

# query set
qs1 = HistoryMaand.objects.filter(timestamp__gte=datetime_query)
...
# drop columns
df1HistYear2.columns
df1HistYear2.drop(columns=['act_verbr_gas', 'act_verbr_kw_170', 'act_gelvr_kw_270',],inplace=True)

# drop first line with NAN values and incomplete last line
df1HistYear2 = df1HistYear2[1:-1]
...
print("Converted to history: years")